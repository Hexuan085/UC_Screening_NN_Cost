...
line_flow_limit_l = np.ones((1, num_of_lines-1), dtype=float) * 3  # Upper\lower bound of line flow of the relaxed LP
...
percentage = 0  # load variation range $r$
...
for i in range(num_samples):
    ...
    for m in range(num_of_lines):
        ...
        constraint_l_max = [q_l_max == b * x_l_max - load_l_max.reshape(-1, ),
                            ...
                            u_l_max <= 1, u_l_max >= 0,  # Relaxed integer variables
        ...
        constraint_l_min = [q_l_min == b * x_l_min - load_l_min.reshape(-1,),
                            ...
                            u_l_min <= 1, u_l_min >= 0,  # Relaxed integer variables
        ...
        constraint_l_max_bound = [q_l_max_bound == b * x_l_max_bound - load_l_max_bound.reshape(-1, ),
                                  ...
                                  u_l_max_bound <= 1, u_l_max_bound >= 0,  # Relaxed integer variables
                                  a_ln_l * q_l_max_bound >= -line_flow_limit_l.reshape(-1, ),
                                  line_flow_limit_l.reshape(-1, ) >= a_ln_l * q_l_max_bound,
                                  C * x_l_max_bound <= 21.0932,  # Cost constraint
                                  ]
        ...
        constraint_l_min_bound = [q_l_min_bound == b * x_l_min_bound - load_l_min_bound.reshape(-1, ),
                                  ...
                                  u_l_min_bound <= 1, u_l_min_bound >= 0,  # Relaxed integer variables
                                  a_ln_l * q_l_min_bound >= -line_flow_limit_l.reshape(-1, ),
                                  line_flow_limit_l.reshape(-1, ) >= a_ln_l * q_l_min_bound,
                                  C * x_l_min_bound <= 21.0932,  # Cost constraint
                                  ]
        ...
        if abs(l_min_bound.value) >= 3:
            num_con_bound[i] += 1
            sta_line_bound_min.append(m)  # Recording the index of binding constraint.
            print('m', m)
...
for j in range(num_samples):
    l_status = [i for i, e in enumerate(line_flow_all[j]) if abs(e) > 2.9999]
