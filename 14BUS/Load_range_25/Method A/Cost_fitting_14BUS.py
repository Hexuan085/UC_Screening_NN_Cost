...
model = keras_model_dnn(input_dim=num_of_buses, output_dim=1)  # Neural network used in our work
...
loss = tf.reduce_mean(tf.square(predictions - y_val))  #Loss function
...
with tf.Session() as sess:
    ...
    status_limit = 0  # The number of binding bounds in the projection process
    k = 0
    percentage = 0.25
    l_center = 15 / num_of_buses
    while (cost_max_0 - cost_max_1) >= 0.0001 and k <= 5000:
        k += 1
        cost_max_1 = cost_max_0
        grad_0 = sess.run(grad_pre, feed_dict={x_val: load})  # Calculate gradient
        grad_0 = np.array(grad_0)
        index = np.argsort(grad_0)  # Sort the elements of gradient
        index = index[0][0]
        grad_status = 0  # Status to record if the element in gradient to be increased is found

        for i in index[::-1]:  # Find the element from largest to smallest
            if (load[0][i]) * (max_valuex[i] - min_valuex[i]) + min_valuex[i] + e <= l_center + percentage * l_center:  # Ensure that the selected elment is still in the load region
                load[0][i] = (((load[0][i]) * (max_valuex[i] - min_valuex[i]) + min_valuex[i] + e) - min_valuex[i]) / (
                            max_valuex[i] - min_valuex[i])  # Move in the direction of selected element
                grad_status = 1
                break
            else:
                status_limit += 1
        for j in index:  # Find the element from largest to smallest
            if grad_status == 0:
                break
            elif load[0][j] * (max_valuex[j] - min_valuex[j]) + min_valuex[j] - e >= l_center - percentage * l_center:  # Ensure that the selected elment is still in the load region
    ...
    print(cost_max_0)  # Output the value of upper bound
    print(k)  # Output the number of iteration
    print(status_limit)  # Output the number of binding bounds in the projection process
    print(max(cost_record))  # Output the largest predicted cost
