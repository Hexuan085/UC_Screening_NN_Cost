...
#  load the PTDF, generator information, marginal cost, binding constraints indexes
...
reduce_max = sta_line_max.shape[1]  # Generate the reduced line flow constraints
...
# Generate the reduced PTDF matrix
a_ln_max = []
a_ln_min = []
a_ln_min_bound = []
a_ln_max_bound = []

for l_index in sta_line_max:  # Upper bound coefficient matrix of line flow for the standard form
    a_ln_max.append(a_ln[l_index][:])

for l_index in sta_line_min:  # Upper bound coefficient matrix of line flow for the cost-driven form
    a_ln_min.append(a_ln[l_index][:])

for l_index in sta_line_bound_min:  # Lower bound coefficient matrix of line flow for the standard form
    a_ln_min_bound.append(a_ln[l_index][:])

for l_index in sta_line_bound_max:  # Lower bound coefficient matrix of line flow for the cost-driven form
...
for i in range(num_samples):
    ...
    # Model and solve original UC problem
    ...
    # Model and solve reduced UC problem obtained by standard form
    start_time = time.time()
    ...
    # Model and solve reduced UC problem obtained by cost-driven form
    start_time = time.time()
...
print('time_0, time_1, time_2', time_0, time_1, time_2)
