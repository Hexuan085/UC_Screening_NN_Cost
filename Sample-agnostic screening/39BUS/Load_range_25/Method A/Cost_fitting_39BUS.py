...
with tf.Session() as sess:
    ...
    while (cost_max_0 - cost_max_1) >= 0.0001 and k <= 5000:
        ...
        for i in index[::-1]:   # Find the element from largest to smallest
            if (load[0][i]) * (max_valuex[i] - min_valuex[i]) + min_valuex[i] + e <= 0.96:  # Ensure that the selected elment is still in the load region
        ...
        for j in index:  # Find the element from largest to smallest
            if grad_status == 0:
                break
            elif load[0][j] * (max_valuex[j] - min_valuex[j]) + min_valuex[j] - e >= 0.56:  # Ensure that the selected elment is still in the load region
