...
percentage = 1
l_center = 30 / num_buses
infeasible = 0

# Load the model parameters
model = keras_model_dnn(input_dim=num_buses, output_dim=1)
model.load_weights(r'model39.h5')

num = 0
for i in range(num_samples):

    # load_status = -load_status
    # load_left = np.ones((1, 20), dtype=float) * l_center
    # load_right = np.ones((1, 19), dtype=float) * l_center
    # # print(load_left)
    # load_random_left = np.random.uniform(0, percentage * l_center, (1, 20))
    ...
    max_valuey = 65.16485
    min_valuey = 22.271053
    cost_pre = cost_pre * (max_valuey - min_valuey) + min_valuey
    print(cost_pre)

    if cost_pre + 0.03 * cost_pre < cost.value:
        infeasible += 1
        continue
    ...
    for m in range(num_of_lines):
        ...
        constraint_l_max_bound = [q_l_max_bound == b * x_l_max_bound - load_l_max_bound.reshape(-1, ),
                                  ...
                                  C * x_l_max_bound <= cost_pre+cost_pre*0.03,
                                  ]
